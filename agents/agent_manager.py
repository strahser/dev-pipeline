...
SUBPROMPT = """Ты — субагент-исполнитель конвейера dev-pipeline.
...
Требования:
1. Прочитай файл задачи целиком (контекст, требования, границы).
2. Выполни требования. Доказательства — реальные выводы: сборка EXIT 0,
   тесты (dotnet test / vstest N/M), grep-выводы, пути файлов, коммиты agent/{task_id}.
3. Отчёт ПО-РУССКИ в {report} по шаблону протокола
   (секции: «Что было не так», «Что сделано», «Доказательства»,
   «Числа до/после», «Открытые вопросы», «Как пересобрать/проверить»).
4. После отчёта: в шапке задачи замени 'статус: in_progress' на 'статус: done_report'.
5. Коммит: git commit -m "agent/{task_id}: отчёт исполнителя".
6. Не имитируй действия; при невозможности — честно blocked/NEED_DATA в отчёте.
7. Не создавай субагентов, не меняй чужие файлы, не закрывай задачу сам.
"""
...
def _run_batch(cfg, ids, args):
    ...
    if args.sequential:
        for tid in ids:
            ...
            results[tid] = rc
            print(f"  [manager] {tid}: rc={rc}, отчёт={'есть' if report.exists() else 'НЕТ'}")
    else:
