...
- **Права записи — консервативный дефолт read**: базовый профиль
  `.opencode/permissions.json` — read (edit deny). Режим write на время задачи
  выдают владелец или менеджер-агент динамически: `PUT/POST
  /api/projects/{name}/permissions {"mode":"read"|"write"}` либо
  `python -m pipeline.cli perms <project> read|write`. Сессии подхватывают профиль
  при старте порции (tui_cycle/up). По завершении задачи права возвращаются в read.
