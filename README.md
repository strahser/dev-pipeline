...
Сервер поднимается через `start-server.bat` (или напрямую:
`python -X utf8 -m server --port 8787`); агентов по проектам — кнопками в панели
(«▶ Поднять проект», «🧭 Контролёр», «🤖 Исполнитель», «🛡 Менеджер»).
